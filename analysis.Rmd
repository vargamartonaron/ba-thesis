...
```{r freq ce model summary}
f_ce <- readRDS("models/freq_ce_model.rds")
f_ce_summary <- summary(f_ce)
```

```{r freq cse model summary}
f_cse <- readRDS("models/freq_cse_model.rds")
f_cse_summary <- summary(f_cse)

```

```{r freq cse conditions model summary}
f_cse_conditions <- readRDS("models/freq_cse_conditions_model.rds")

```

```{r motoric summary}
# f_cse_motoric <- readRDS("models/freq_cse_motoric_model.rds")
```
...
#summary(h_ce)
ce_effect_posterior <- pp_check(h_ce, type = "dens_overlay") + labs(y = "Posterior distribution of model and observed")
#ce_effect_estimates <- plot(conditional_effects(h_ce), theme = theme_apa(base_family = "Times"))
```

```{r h_cse model investigation}
#cse_effect_posterior <- plot(h_cse, variable = "congruencyincongruent:prev_congruencyincongruent")
