...
model_dir <- "models"
h0_priors <- c(
  set_prior("normal(0, 15)", class = "b", coef = congruency),
  set_prior("normal(0, 15)", class = "b", coef = prev_congruency),
  set_prior("student_t(3, 0, 10)", class = "sd"),
  set_prior("lkj(2)", class = "cor")
)

h0_formula <- bf(rt ~ congruency * prev_congruency + (1 + congruency + prev_congruency | participant_id))

null_fit <- brm(formula = h0_formula, data = train_df, prior = h0_priors, seed = 1234, sample_prior = "yes", save_model = file.path(model_dir, "null_model.stan"), file = file.path(model_dir, "null"))
```


```{r specifying models to test h1}
h1_priors <- c(
  set_prior("normal(0, 15)", class = "b", coef = congruency),
  set_prior("normal(0, 15)", class = "b", coef = prev_congruency),
  set_prior("normal(0, 15)", class = "b", coef = condition_positive, hb = 0),
  set_prior("normal(0, 15)", class = "b", coef = condition_negative, lb = 0),
  set_prior("normal(0, 15)", class = "b", coef = condition_neutral),
  set_prior("student_t(3, 0, 10)", class = "sd"),
  set_prior("lkj(2)", class = "cor")
)

h1_formula <- bf(rt ~ congruency * prev_congruency * 
              (condition_positive + condition_negative + condition_neutral) + 
              (1 + congruency | participant_id))
h1_fit <- brm(formula = h1_formula, data = train_df, prior = h1_priors, seed = 1234, sample_prior = "yes", save_model = file.path(model_dir, "h1_model.stan"), file = file.path(model_dir, "h1"))
```
